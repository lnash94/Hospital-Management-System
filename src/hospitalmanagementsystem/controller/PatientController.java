...
public class PatientController {
    ...
    public static ResultSet getPatient(Patient patient)throws ClassNotFoundException,SQLException{
        conn =DbConnector.connectDb();
        String q = "SELECT * FROM patient WHERE p_id='"+patient.getP_id()+"' OR p_name ='"+patient.getName()+"'";
        Statement stm =conn.createStatement();
    ...
    //addmited patient.
     public static int addInPatient(InPatient inpatient)throws ClassNotFoundException,SQLException{
      ...
      public  static ResultSet getPatentId(Patient patient) throws ClassNotFoundException,SQLException{
        conn =DbConnector.connectDb();
        String q = "SELECT p_id FROM patient ORDER BY p_id DESC LIMIT 1";
        Statement stm =conn.createStatement();
      ...
      //update patient
     public static int updatePatient(Patient patient)throws ClassNotFoundException,SQLException{
      
       
            conn = DbConnector.connectDb();
            String q = "UPDATE patient SET p_name='"+patient.getName()+"',p_gender='"+patient.getGender()+"',p_nic='"+patient.getNic()+"',p_dob='"+patient.getDob()+"',p_contacte='"+patient.getContactno()+"',p_address='"+patient.getAddress()+"' WHERE p_id='"+patient.getP_id()+"'";
            Statement stm = conn.createStatement();
            int executeUpdate = stm.executeUpdate(q);
